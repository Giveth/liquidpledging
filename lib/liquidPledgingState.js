class LiquidPledgingState {
  ...
  $getPledge(idPledge) {
    ...
    return this.$lp.getPledge(idPledge).then(res => {
      ...
      for (let i = 1; i <= res.nDelegates; i += 1) {
        promises.push(this.$lp.getPledgeDelegate(idPledge, i).then(r => ({
          id: r.idDelegate,
          addr: r.addr,
          name: r.name,
          url: r.url
  ...
  $getAdmin(idAdmin) {
    const admin = {};
    return this.$lp.getPledgeAdmin(idAdmin).then(res => {
      if (res.adminType === '0') {
        admin.type = 'Giver';
  ...
  getState() {
    const getPledges = () => this.$lp.numberOfPledges().then(nPledges => {
      const promises = [];
    ...
    const getAdmins = () => this.$lp.numberOfPledgeAdmins().then(nAdmins => {
      const promises = [];
