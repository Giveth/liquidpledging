...
describe('LiquidPledging Ether test', function() {
  this.timeout(0);
  let ganache;
  let web3;
  let accounts;
  ...
  const zeroXzero = '0x0000000000000000000000000000000000000000';
  ...
  it('Should make a donation in ether', async () => {
    const r = await liquidPledging.donate(1, 1, {
      value: utils.toWei('1'), 
      from: giver1,
      $extraGas: 100000,
    });
  ...
  it('Delegate should assign to project1', async () => {
    ...
    assert.equal(res3.token, zeroXzero);
  ...
  it('Giver should change his mind and assign half of it to project2', async () => {
    ...
    assert.equal(res4.token, zeroXzero);
  ...
  it('After the time, the project1 should be able to spend part of it', async () => {
    ...
    assert.equal(res5.token, zeroXzero);
    ...
    assert.equal(res6.token, zeroXzero);
  ...
  it('Should collect the ether', async () => {
    ...
    const finalBalance = await web3.eth.getBalance(vault.$address);
    const collected = utils.fromWei(utils.toBN(initialBalance).sub(utils.toBN(finalBalance)));
    ...
    assert.equal(res7.token, zeroXzero);
